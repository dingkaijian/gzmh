...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a style="color:#2fa4e7;">通知通告</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="article" action="${ctx}/sys/man/tztg/" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<input id="categoryId" name="category.id" class="input-small" type="hidden" value="${article.category.id}">
	...
	<ul>
		...
		<c:forEach items="${page.list}" var="article">
			<li style="border-bottom: 1px dashed #7cc7ef;color: #444;list-style: none;line-height:3;">
				<span><a href="${pageContext.request.contextPath}${fns:getFrontPath()}/view-${article.category.id}-${article.id}${fns:getUrlSuffix()}" target="_blank" style="padding-left: 20px;">${article.title}</a></span>
				<span style="float:right;margin-right: 20px;"><fmt:formatDate value="${article.updateDate}" type="both" pattern="yyyy-MM-dd"/></span>
			</li>
		</c:forEach>
	</ul>
