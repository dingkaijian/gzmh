...
<body>
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="sys:appconfig:sys"><li  class="active"><a href="${ctx}/sys/appconfig/form2/">系统配置策略<shiro:hasPermission name="sys:appconfig:sys">${not empty appconfig.id?'修改':'添加'}</shiro:hasPermission></a></li></shiro:hasPermission>
		</ul></br>
	<form:form id="inputForm" modelAttribute="appconfig" action="${ctx}/sys/appconfig/saveBySys/"  method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>
		<div class="control-group">
			<label class="control-label">策略名称:</label>
			<div class="controls">
				<form:input path="configname" htmlEscape="false" name="configname" id="configname" value="${appconfig.configname}"/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<select class="input-medium" name="userpermission">
				<option value="sys">系统配置策略</option>
			</select>
			</div>
		</div>
		<div class="form-actions">
			<shiro:hasPermission name="sys:appconfig:sys"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
