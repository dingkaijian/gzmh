...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/xtjh/xtjhInfo/messageGroup">对话列表</a></li>
		<li class="active"><a>消息列表</a></li>
		<shiro:hasPermission name="xtjh:xtjhInfo:edit">
			<li><a href="${ctx}/xtjh/xtjhInfo/form">发送信息</a></li>
		</shiro:hasPermission>
	</ul>
	...
	<table id="contentTable"
		...
		<tbody>
			<c:forEach items="${page.list}" var="xtjhInfo">
				...
				<td>${xtjhInfo.to.name}</td>
					<td><c:if test="${xtjhInfo.bySms == 1}">[短信发送]</c:if>
						<c:if test="${xtjhInfo.byEmail == 1}">[邮件发送]</c:if><a
						href="${ctx}/xtjh/xtjhInfo/form?id=${xtjhInfo.id}">${xtjhInfo.content}</a>
					</td>
					<td><fmt:formatDate value="${xtjhInfo.createDate}"
							pattern="yyyy-MM-dd HH:mm:ss" /></td>
					<%-- <shiro:hasPermission name="xtjh:xtjhInfo:edit"><td>
    				<a href="${ctx}/xtjh/xtjhInfo/form?id=${xtjhInfo.id}">修改</a>
